...
3. **BEFORE calling the task done** — run `mcp__rollout-server__assert_dev_checklist` with `{ feature }`. **PARK on any block.** Also run `detect_rollout_ready` + `reconcile` phase `build`.
...
3. **BEFORE done** call `mcp__rollout-server__assert_dev_checklist` with `{ feature }` — PARK on any block. Also `reconcile` phase `build`.

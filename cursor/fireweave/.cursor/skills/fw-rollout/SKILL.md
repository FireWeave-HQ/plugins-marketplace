...
- **`fireweave-cloud-bridge`** — stdio MCP shim that translates MCP
  ...
  surface as `mcp_fireweave-cloud-bridge_<name>`. The bridge replaces
...
- **`rollout-server`** — local stdio. Owns operations that genuinely require
  ...
  inspect local code. Tools surface as `mcp_rollout-server_<name>`.
...
`mcp_rollout-server_read_lockfile` and branch on the result.
...
- `lastStep === 'register' && state.rolloutId` — jump to the **post-register
  status path**: call `mcp_fireweave-server-proxy_get_rollout_status` with the
...
1. Call `mcp_fireweave-server-proxy_get_rollout_status(rolloutId)`.
  ...
  - No, abort and start fresh
   ...
   - **Yes, update** → this is a **Configuration step** — call via `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`mcp_fireweave-server-proxy_`, `update_participant_sha`).
   ...
   - **No, abort and start fresh** → call `mcp_rollout-server_clear_lockfile`;
...
1. Call `mcp_fireweave-server-proxy_get_rollout_status(rolloutId)` and read
...
- Options:
  ...
  - No, keep the registered spec
   ...
   - **Yes, update the spec** → this is a **Configuration step** — call via
     `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`mcp_fireweave-server-proxy_`, `update_rollout_spec`).
     2. Call `mcp_rollout-server_guarded_call` with
...
via `mcp_rollout-server_write_lockfile` so the next interruption resumes
from the right place.** After `register_rollout` succeeds at Step 9, call
`mcp_rollout-server_clear_lockfile` to mark the work complete.
...
memory. Cloud-routed tools surface as `mcp_fireweave-cloud-bridge_<name>`;
...
`mcp_rollout-server_<name>` during the deprecation window.
...
Call `mcp_rollout-server_list_registered_tools` and
`mcp_fireweave-server-proxy_list_registered_tools` (the latter
...
1. **Multi-rollout coexistence (D21).** Call
   `mcp_fireweave-server-proxy_list_open_rollouts` with the projectId. If
...
2. **Multi-repo coordination (D6).** Call
   `mcp_fireweave-server-proxy_list_project_repos` with the projectId. If
...
3. **Capability discovery.** Call
   `mcp_fireweave-server-proxy_get_project_capabilities` to enumerate which
   ...
   no provider, present a per-capability missing-config gate (`a stop-and-ask user prompt`
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-ALERTS`** — STOP and ask the user to pick one (or drive via MCP Elicitation in the server):
...
4. **Environment selection (D13).** Call
   `mcp_fireweave-server-proxy_list_project_environments`.
...
**Gate `GATE-0.2-ENVIRONMENT-CHOICE`** — STOP and ask the user to pick one (or drive via MCP Elicitation in the server):
...
6. **Rollout history defaults.** Call
   `mcp_fireweave-server-proxy_get_rollout_history` and
   `mcp_fireweave-server-proxy_get_rollout_learnings` for this project. v1
...
2. Pass the diff text into `mcp_rollout-server_extract_diff_surface` —
...
Call `mcp_fireweave-cloud-bridge_get_recommendation_data` with
...
`mcp_fireweave-server-proxy_get_project_capabilities`. For any capability still
...
1. Call `mcp_fireweave-server-proxy_propose_sdk_install` with
...
2. If `needsInstall === true`, call
   `mcp_fireweave-server-proxy_generate_sdk_init_module` with
...
`mcp_rollout-server_write_lockfile` again with `diffApplied: true`. From
...
Run all seven `mcp_rollout-server_verify_*` tools, in order. Each returns
...
1. Resolve the underlying tool name and server prefix
   (`mcp_fireweave-server-proxy_`, `register_rollout`).
2. Call `mcp_rollout-server_guarded_call` with
...
Immediately call `mcp_rollout-server_write_lockfile` with `{ lastStep:
...
- **Never** write to `.fireweave/rollout.config.json` except via
  `mcp_rollout-server_write_preferences`.

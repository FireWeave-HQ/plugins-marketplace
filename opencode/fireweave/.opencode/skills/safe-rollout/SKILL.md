...
- **`fireweave-cloud-bridge`** — stdio MCP shim that translates MCP
  ...
  surface as `fireweave-cloud-bridge_<name>`. The bridge replaces
...
- **`rollout-server`** — local stdio. Owns operations that genuinely require
  ...
  inspect local code. Tools surface as `rollout-server_<name>`.
...
`rollout-server_read_lockfile` and branch on the result.
...
- `lastStep === 'register' && state.rolloutId` — jump to the **post-register
  status path**: call `fireweave-server-proxy_get_rollout_status` with the
...
1. Call `fireweave-server-proxy_get_rollout_status(rolloutId)`.
  ...
  - No, abort and start fresh
   ...
   - **Yes, update** → this is a **Configuration step** — call via `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`fireweave-server-proxy_`, `update_participant_sha`).
   ...
   - **No, abort and start fresh** → call `rollout-server_clear_lockfile`;
...
1. Call `fireweave-server-proxy_get_rollout_status(rolloutId)` and read
...
- Options:
  ...
  - No, keep the registered spec
   ...
   - **Yes, update the spec** → this is a **Configuration step** — call via
     `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`fireweave-server-proxy_`, `update_rollout_spec`).
     2. Call `rollout-server_guarded_call` with
...
via `rollout-server_write_lockfile` so the next interruption resumes
from the right place.** After `register_rollout` succeeds at Step 9, call
`rollout-server_clear_lockfile` to mark the work complete.
...
memory. Cloud-routed tools surface as `fireweave-cloud-bridge_<name>`;
...
`rollout-server_<name>` during the deprecation window.
...
Call `rollout-server_list_registered_tools` and
`fireweave-server-proxy_list_registered_tools` (the latter
...
1. **Multi-rollout coexistence (D21).** Call
   `fireweave-server-proxy_list_open_rollouts` with the projectId. If
...
2. **Multi-repo coordination (D6).** Call
   `fireweave-server-proxy_list_project_repos` with the projectId. If
...
3. **Capability discovery.** Call
   `fireweave-server-proxy_get_project_capabilities` to enumerate which
   ...
   no provider, present a per-capability missing-config gate (`the `question` tool`
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-ALERTS`** — use the `question` tool:
...
4. **Environment selection (D13).** Call
   `fireweave-server-proxy_list_project_environments`.
...
**Gate `GATE-0.2-ENVIRONMENT-CHOICE`** — use the `question` tool:
...
6. **Rollout history defaults.** Call
   `fireweave-server-proxy_get_rollout_history` and
   `fireweave-server-proxy_get_rollout_learnings` for this project. v1
...
2. Pass the diff text into `rollout-server_extract_diff_surface` —
...
Call `fireweave-cloud-bridge_get_recommendation_data` with
...
`fireweave-server-proxy_get_project_capabilities`. For any capability still
...
1. Call `fireweave-server-proxy_propose_sdk_install` with
...
2. If `needsInstall === true`, call
   `fireweave-server-proxy_generate_sdk_init_module` with
...
`rollout-server_write_lockfile` again with `diffApplied: true`. From
...
Run all seven `rollout-server_verify_*` tools, in order. Each returns
...
1. Resolve the underlying tool name and server prefix
   (`fireweave-server-proxy_`, `register_rollout`).
2. Call `rollout-server_guarded_call` with
...
Immediately call `rollout-server_write_lockfile` with `{ lastStep:
...
- **Never** write to `.fireweave/rollout.config.json` except via
  `rollout-server_write_preferences`.

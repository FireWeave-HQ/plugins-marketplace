...
- Call `mcp__rollout-server__detect_rollout_ready` and `mcp__rollout-server__reconcile` with `phase: "build"`.
...
1. Resolve platform path: `FIREWEAVE_PLATFORM_PATH` env → else walk parents for `fireweaveai-platform` → else `AskUserQuestion` for the directory containing `packages/fw-plugins/.../rollout-server`.

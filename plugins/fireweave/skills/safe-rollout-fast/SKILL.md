...
| Step | Action |
|---|---|
| **0.2 — Dev checklist (fail closed)** | Run `mcp__rollout-server__assert_dev_checklist` with `{ feature }`. On any **block** → PARK and refuse to register. Backfill during ship is forbidden. |

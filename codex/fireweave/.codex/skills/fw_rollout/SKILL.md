...
- **`fireweave-cloud-bridge`** — stdio MCP shim that translates MCP
  ...
  surface as `mcp__fireweave_cloud_bridge__<name>`. The bridge replaces
...
- **`rollout-server`** — local stdio. Owns operations that genuinely require
  ...
  inspect local code. Tools surface as `mcp__rollout_server__<name>`.
...
`mcp__rollout_server__read_lockfile` and branch on the result.
...
- `lastStep === 'register' && state.rolloutId` — jump to the **post-register
  status path**: call `mcp__fireweave_server_proxy__get_rollout_status` with the
...
1. Call `mcp__fireweave_server_proxy__get_rollout_status(rolloutId)`.
  ...
  - No, abort and start fresh
   ...
   - **Yes, update** → this is a **Configuration step** — call via `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`mcp__fireweave_server_proxy__`, `update_participant_sha`).
   ...
   - **No, abort and start fresh** → call `mcp__rollout_server__clear_lockfile`;
...
1. Call `mcp__fireweave_server_proxy__get_rollout_status(rolloutId)` and read
...
- Options:
  ...
  - No, keep the registered spec
   ...
   - **Yes, update the spec** → this is a **Configuration step** — call via
     `guarded_call`:
     1. Resolve the underlying tool name and server prefix
        (`mcp__fireweave_server_proxy__`, `update_rollout_spec`).
     2. Call `mcp__rollout_server__guarded_call` with
...
via `mcp__rollout_server__write_lockfile` so the next interruption resumes
from the right place.** After `register_rollout` succeeds at Step 9, call
`mcp__rollout_server__clear_lockfile` to mark the work complete.
...
memory. Cloud-routed tools surface as `mcp__fireweave_cloud_bridge__<name>`;
...
`mcp__rollout_server__<name>` during the deprecation window.
...
Call `mcp__rollout_server__list_registered_tools` and
`mcp__fireweave_server_proxy__list_registered_tools` (the latter
...
1. **Multi-rollout coexistence (D21).** Call
   `mcp__fireweave_server_proxy__list_open_rollouts` with the projectId. If
...
2. **Multi-repo coordination (D6).** Call
   `mcp__fireweave_server_proxy__list_project_repos` with the projectId. If
...
3. **Capability discovery.** Call
   `mcp__fireweave_server_proxy__get_project_capabilities` to enumerate which
...
**Gate `GATE-0.2-CAPABILITY-FALLBACK-ALERTS`** — call `request_user_input` (single-choice; recommended first; "Other" auto-added):
...
4. **Environment selection (D13).** Call
   `mcp__fireweave_server_proxy__list_project_environments`.
...
6. **Rollout history defaults.** Call
   `mcp__fireweave_server_proxy__get_rollout_history` and
   `mcp__fireweave_server_proxy__get_rollout_learnings` for this project. v1
...
2. Pass the diff text into `mcp__rollout_server__extract_diff_surface` —
...
Call `mcp__fireweave_cloud_bridge__get_recommendation_data` with
...
`mcp__fireweave_server_proxy__get_project_capabilities`. For any capability still
...
1. Call `mcp__fireweave_server_proxy__propose_sdk_install` with
...
2. If `needsInstall === true`, call
   `mcp__fireweave_server_proxy__generate_sdk_init_module` with
...
`mcp__rollout_server__write_lockfile` again with `diffApplied: true`. From
...
Run all seven `mcp__rollout_server__verify_*` tools, in order. Each returns
...
1. Resolve the underlying tool name and server prefix
   (`mcp__fireweave_server_proxy__`, `register_rollout`).
2. Call `mcp__rollout_server__guarded_call` with
...
Immediately call `mcp__rollout_server__write_lockfile` with `{ lastStep:
...
- **Never** write to `.fireweave/rollout.config.json` except via
  `mcp__rollout_server__write_preferences`.

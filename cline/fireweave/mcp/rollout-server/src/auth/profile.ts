...
export type ResolveResult =
  | { kind: 'ok'; profile: Profile; source: 'flag' | 'repo' | 'global-default' | 'sole' }
...
export function loadProfile(alias: string, home: string = homedir()): Profile | null {
  try {
...
function loadHomeConfig(home: string): HomeConfig {
  try {
    ...
    return {
      activeProfile: typeof raw.activeProfile === 'string' ? raw.activeProfile : null,
    };
  } catch {
    return {};
  }
...
function loadRepoConfig(cwd: string): RepoConfig | null {
  try {
    ...
    if (!existsSync(path)) return null;
    const raw = JSON.parse(readFileSync(path, 'utf8')) as Record<string, unknown>;
    return { profile: typeof raw.profile === 'string' ? raw.profile : undefined };
  } catch {
    return null;
  }
}
...
export function resolveActiveProfile(opts: ResolveOptions = {}): ResolveResult {
  ...
  if (flagAlias) {
    ...
    return { kind: 'missing', reason: `Profile '${flagAlias}' (from FW_PROFILE) not found in ~/.fireweave/profiles/` };
  }
  ...
  // 5. Missing
  if (all.length === 0) {
    return { kind: 'missing', reason: 'No Fireweave profiles found. Run `fw login` (or `fw init` for guided setup).' };
  }
  return {
    kind: 'missing',
